...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'SELECT t.id, t.description, u.username, t.completed, t.created_at FROM todo t JOIN user u on t.created_by = u.id WHERE t.id = %s',
        (id,)
    )
    todo = c.fetchone()

    if todo is None:
        abort(404, "El Todo de {0} no existe.".format(id))
    return todo
